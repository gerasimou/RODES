...
evolve const double evTrans1 [0.0 .. 2.0];
evolve const double evTrans2 [0.0 .. 2.0];
//evolve const double evTrans3 [0.0 .. 1.0];
...
evolve module PM
	...
	[requestH] QL=0 & QH=0 & sp=3 -> evTrans1 : (p'=0) + (1-evTrans1) : (p'=1);
	[requestL] QL=3 & QH=0 & sp=3 -> evTrans2 : (p'=0) + (1-evTrans2) : (p'=1);
	[sleep2wait] p = 1 -> (p'=0);
...
evolve module PM
	...
	[sleep2wait] false -> true;
	[wait2idle] (QL=0 & QH > 0)|(QL=1 & QH > 0)|QL > 1 -> (p'=p);
	[requestH] QL=0 & QH=0 & sp=3 -> evTrans2 : (p'=0) + (1-evTrans1) : (p'=1);
	[requestL] QL=3 & QH=0 & sp=3 -> evTrans3 : (p'=0) + (1-evTrans2) : (p'=1);
	[requestH]! (QL=0 & QH=0 & sp=3) -> (p'=p);
	[requestL]!(QL=3 & QH=0 & sp=3) -> (p'=p);
endmodule
...
evolve const double wait2idle [0.1..2.0];//= 0.454;
...
const double reqL = 0.15; 
const double reqH = 0.25;
