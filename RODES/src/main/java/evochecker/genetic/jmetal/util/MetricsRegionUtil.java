 ...
 **/
public class MetricsRegionUtil {
  ...
  public SolutionSet readNonDominatedSolutionSet(String path) {
	 ...
	 * @return A solution set
	 */
	public void readNonDominatedSolutionSet(String path, SolutionSet solutionSet) {
		try {
			/* Open the file */
			FileInputStream fis   = new FileInputStream(path)     ;
			InputStreamReader isr = new InputStreamReader(fis)    ;
	 ...
	 * @return A solution set
	 */
	public void readNonDominatedRegionSolutionSet(String paretoFrontPath, String paretoFrontSet, SolutionSet solutionSet) {
		String frontStr = "";
		String setStr   = "";
		try {
			/* Open the file */
			FileInputStream fis   	= new FileInputStream(paretoFrontPath);
			InputStreamReader isr 	= new InputStreamReader(fis);
			BufferedReader br      	= new BufferedReader(isr);	      	     

			FileInputStream fis2   	= new FileInputStream(paretoFrontSet);
			InputStreamReader isr2 	= new InputStreamReader(fis2);
			BufferedReader br2      = new BufferedReader(isr2);	      	     

			frontStr = br.readLine();
			setStr   = br2.readLine();
			while (frontStr!= null && !frontStr.equals("\n") && !frontStr.equals("")){
				String input[] 	= frontStr.split("\\:|\\,");
				
				String input2[]	= setStr.split("\\:|\\,");
				
//				System.out.println(Arrays.toString(input));
				
//				StringTokenizer st = new StringTokenizer(aux,":,");
//				int i = 0;
				RegionSolution solution = new  RegionSolution(2, 
												new double[][]{{-Double.parseDouble(input[1]), -Double.parseDouble(input[0])},
															   {-Double.parseDouble(input[3]), -Double.parseDouble(input[2])}},
											 2, new double[][]{{Double.parseDouble(input2[0]), Double.parseDouble(input2[1])},
															   {Double.parseDouble(input2[2]), Double.parseDouble(input2[3])}});
//				solution.setObjectiveBounds(0, Double.parseDouble(input[0]), Double.parseDouble(input[1]));
//				solution.setObjectiveBounds(1, Double.parseDouble(input[2]), Double.parseDouble(input[3]));
//				
//				while (st.hasMoreTokens()) {
//					double value = new Double(st.nextToken());
//					solution.setObjective(i,value);
...
//				}
			...
			System.err.println("jmetal.qualityIndicator.util.readNonDominatedSolutionSet: "+paretoFrontPath +"\t"+ frontStr);
			e.printStackTrace();
		}
	}
